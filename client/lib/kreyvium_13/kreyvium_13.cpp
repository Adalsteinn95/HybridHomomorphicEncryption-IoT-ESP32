...
std::vector<uint8_t> Kreyvium13::encrypt(std::vector<uint8_t> plaintext,
                                         size_t bits) const {
...
std::vector<uint8_t> Kreyvium13::decrypt(std::vector<uint8_t> ciphertext,
                                         size_t bits) const {
  keyblock key = convert_key(secret_key);
...
void Kreyvium13::prep_one_block() const {}

keystreamblock Kreyvium13::keystream(const keyblock& key,
  ...
  for (size_t i = 0; i < warmup + KREYVIUM13_PARAMS.plain_size_bits; i++) {
    bool t4 = inner_key[KREYVIUM13_PARAMS.key_size_bits - 1];
...
void Kreyvium13::init(const keyblock& key, const ivblock& iv, stateblock& state,
  ...
  for (size_t i = 0; i < KREYVIUM13_PARAMS.key_size_bits; i++) {
    inner_key[i] = key[KREYVIUM13_PARAMS.key_size_bits - 1 - i];
  }
}
