#include "handlers/Filip1280TfheHandler.h"
...
const std::string &Filip1280TfheHandler::getResultTopic() const { return result_topic; }
const std::string &Filip1280TfheHandler::getWorkerName() const { return worker_name; }

std::vector<uint8_t> Filip1280TfheHandler::getSecretKey()
{
...
Filip1280TfheHandler::Filip1280TfheHandler()
{
...
void Filip1280TfheHandler::loadContextAndKeys()
{
    std::cout << "[" << worker_name << "] Initializing filip1280_128_TFHE implementation..." << std::endl;
    std::vector<uint8_t> secret_key = getSecretKey();
    int security_level = 128;

    try
    {
        tfhe_impl_ptr = std::make_unique<FILIP_1280::FiLIP_1280_TFHE>(secret_key, security_level);
    }
    catch (const std::exception &e)
    {
        tfhe_impl_ptr.reset();
        throw std::runtime_error(worker_name + " filip1280_128_TFHE construction/TFHE setup failed: " + std::string(e.what()));
    }

    std::cout << "[" << worker_name << "] filip1280_128_TFHE instance created and TFHE context/keys loaded." << std::endl;
}

std::string Filip1280TfheHandler::processMessage(const std::string &ciphertext_hex_payload)
{
    ...
    try
    {
        ...
        std::cout << "[STEP 4] Encrypting filip1280 Key Homomorphically..." << std::endl;
        ...
        std::cout << "[RESULT] filip1280 final result (hex): " << final_result_hex << std::endl;
        ...
        saveMeasurements(
            "measurement/filip1280_tfhe_measurement.csv", // Use correct filename for each handler
            // --- Timers (ms) ---
            keyEncryptTime,
            heDecryptTime,
            heSumTime,
    ...
    catch (const std::exception &e)
    {
        ...
        totalProcessTime = std::chrono::duration_cast<std::chrono::milliseconds>(total_end_time - total_start_time);
        saveMeasurements(
            "measurement/filip1280_tfhe_measurement.csv",
            // --- Timers (ms) ---
            keyEncryptTime,
            heDecryptTime,
            heSumTime,
