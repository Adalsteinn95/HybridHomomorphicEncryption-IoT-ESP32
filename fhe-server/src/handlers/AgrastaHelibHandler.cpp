...
std::vector<uint8_t> AgrastaHelibHandler::getSecretKey()
{
    std::cout << "[" << worker_name << " WARN] Using a hardcoded 16-byte secret key for AGRASTA." << std::endl;
...
AgrastaHelibHandler::AgrastaHelibHandler() : worker_name("AGRASTA_HELIB_WORKER")
{
...
void AgrastaHelibHandler::createContextAndKeys()
{
    long p_param_test = 2;
    long r_param_test = 1;
...
std::string AgrastaHelibHandler::processMessage(const std::string &ciphertext_hex_payload)
{
    ...
    try
    {
        ...
        std::unique_ptr<AGRASTA::AGRASTA_128_HElib> helib_impl_ptr;
        unsigned long L = 520;
        unsigned long c = 2;
        std::shared_ptr<helib::Context> shared_context = std::shared_ptr<helib::Context>(std::move(context_));
        try
        {
            helib_impl_ptr = std::make_unique<AGRASTA::AGRASTA_128_HElib>(secret_key, shared_context, L, c);
        }
        catch (const std::exception &e)
        {
        ...
        saveMeasurements(
            "measurement/agrasta_helib_measurement.csv", // Use correct filename for each handler
            // --- Timers (ms) ---
            keyEncryptTime,
            heDecryptTime,
            heSumTime,
    ...
    catch (const std::exception &e)
    {
        ...
        totalProcessTime = std::chrono::duration_cast<std::chrono::milliseconds>(total_end_time - total_start_time);
        saveMeasurements(
            "measurement/agrasta_helib_measurement.csv",
            // --- Timers (ms) ---
            keyEncryptTime,
            heDecryptTime,
            heSumTime,
