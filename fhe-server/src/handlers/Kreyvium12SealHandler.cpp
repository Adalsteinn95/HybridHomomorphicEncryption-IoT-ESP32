#include "handlers/Kreyvium12SealHandler.h"
#include "ciphers/kreyvium_12_seal.h"
#include "Utils.h"
...
const std::string &Kreyvium12SealHandler::getResultTopic() const { return result_topic; }
const std::string &Kreyvium12SealHandler::getWorkerName() const { return worker_name; }

std::vector<uint8_t> Kreyvium12SealHandler::getSecretKey()
{
...
Kreyvium12SealHandler::Kreyvium12SealHandler() : worker_name("KREYVIUM12_SEAL_WORKER")
{
...
void Kreyvium12SealHandler::loadContextAndKeys()
{
    ...
    {
        throw std::runtime_error("Failed to create SEALContext for Kreyvium-12: " + std::string(e.what()));
    }
    if (!context->parameters_set())
        throw std::runtime_error("Kreyvium-12 SEAL context parameters check failed.");
    ...
    std::cout << "[" << worker_name << "] SEAL Context, Evaluator & Keys created." << std::endl;
}

std::string Kreyvium12SealHandler::processMessage(const std::string &ciphertext_hex_payload)
{
    ...
    try
    {
        ...
        std::cout << "[STEP 1] Processing Input Ciphertext..." << std::endl;
        ...
        std::cout << "[MEASUREMENT] Input Ciphertext Size(B): " << inputCiphertextSizeBytes << std::endl;

        std::cout << "[STEP 2] Getting Secret Key..." << std::endl;
        std::vector<uint8_t> secret_key = getSecretKey();
        if (secret_key.empty())
            throw std::runtime_error(worker_name + " Secret key retrieval failed.");

        std::cout << "[STEP 3] Initializing KREYVIUM_12_SEAL..." << std::endl;
        std::unique_ptr<KREYVIUM_12::KREYVIUM12_SEAL> seal_impl_ptr;
        try
        {
            seal_impl_ptr = std::make_unique<KREYVIUM_12::KREYVIUM12_SEAL>(secret_key, context);
        }
        catch (const std::exception &e)
        {
            throw std::runtime_error(worker_name + " KREYVIUM_12_SEAL construction failed: " + std::string(e.what()));
        }
        if (!seal_impl_ptr)
        {
            throw std::runtime_error(worker_name + " Failed to create KREYVIUM_12_SEAL instance (nullptr).");
        }
        ...
        std::cout << "[STEP 4] Encrypting Kreyvium12 Key Homomorphically..." << std::endl;
        ...
        std::cout << "[MEASUREMENT] Enc Key Size(B): " << encryptedKeySizeBytes << " (Measurement Unavailable)" << std::endl;
        ...
        std::cout << "[RESULT] Kreyvium12 final result (hex): " << final_result_hex << std::endl;
        ...
        saveMeasurements(
            "measurement/kreyvium12_seal_measurement.csv", // Use correct filename for each handler
            // --- Timers (ms) ---
            keyEncryptTime,
            heDecryptTime,
            heSumTime,
    ...
    catch (const std::exception &e)
    {
        ...
        totalProcessTime = std::chrono::duration_cast<std::chrono::milliseconds>(total_end_time - total_start_time);
        saveMeasurements(
            "measurement/kreyvium12_seal_measurement.csv",
            // --- Timers (ms) ---
            keyEncryptTime,
            heDecryptTime,
            heSumTime,
