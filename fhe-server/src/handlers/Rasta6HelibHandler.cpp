...
std::vector<uint8_t> Rasta6HelibHandler::getSecretKey()
{
...
Rasta6HelibHandler::Rasta6HelibHandler() : worker_name("RASTA6_HELIB_WORKER")
{
...
void Rasta6HelibHandler::createContextAndKeys()
{
    long p_param_test = 2;
    long r_param_test = 1;
...
std::string Rasta6HelibHandler::processMessage(const std::string &ciphertext_hex_payload)
{
    ...
    try
    {
        ...
        std::unique_ptr<RASTA_6::RASTA_128_HElib> helib_impl_ptr;
        unsigned long L = 520;
        unsigned long c = 2;
        std::shared_ptr<helib::Context> shared_context = std::shared_ptr<helib::Context>(std::move(context_));
        try
        {
            helib_impl_ptr = std::make_unique<RASTA_6::RASTA_128_HElib>(secret_key, shared_context, L, c);
        }
        catch (const std::exception &e)
        {
            throw std::runtime_error(worker_name + " RASTA_6 construction failed: " + std::string(e.what()));
        }
        ...
        saveMeasurements(
            "measurement/rasta6_helib_measurement.csv", // Use correct filename for each handler
            // --- Timers (ms) ---
            keyEncryptTime,
            heDecryptTime,
            heSumTime,
    ...
    catch (const std::exception &e)
    {
        ...
        totalProcessTime = std::chrono::duration_cast<std::chrono::milliseconds>(total_end_time - total_start_time);
        saveMeasurements(
            "measurement/rasta6_helib_measurement.csv",
            // --- Timers (ms) ---
            keyEncryptTime,
            heDecryptTime,
            heSumTime,
