#include "handlers/AgrastaSealHandler.h"
#include "ciphers/agrasta_seal.h"
#include "Utils.h"
...
const std::string &AgrastaSealHandler::getResultTopic() const { return result_topic; }
const std::string &AgrastaSealHandler::getWorkerName() const { return worker_name; }

std::vector<uint8_t> AgrastaSealHandler::getSecretKey()
{
...
AgrastaSealHandler::AgrastaSealHandler() : worker_name("AGRASTA_SEAL_WORKER")
{
...
void AgrastaSealHandler::loadContextAndKeys()
{
...
std::string AgrastaSealHandler::processMessage(const std::string &ciphertext_hex_payload)
{
    ...
    try
    {
        ...
        std::unique_ptr<AGRASTA::AGRASTA_128_SEAL> seal_impl_ptr;
        try
        {
            seal_impl_ptr = std::make_unique<AGRASTA::AGRASTA_128_SEAL>(secret_key, context);
        }
        catch (const std::exception &e)
        {
        ...
        saveMeasurements(
            "measurement/agrasta_seal_measurement.csv", // Use correct filename for each handler
            // --- Timers (ms) ---
            keyEncryptTime,
            heDecryptTime,
            heSumTime,
    ...
    catch (const std::exception &e)
    {
        ...
        totalProcessTime = std::chrono::duration_cast<std::chrono::milliseconds>(total_end_time - total_start_time);
        saveMeasurements(
            "measurement/agrasta_seal_measurement.csv",
            // --- Timers (ms) ---
            keyEncryptTime,
            heDecryptTime,
            heSumTime,
