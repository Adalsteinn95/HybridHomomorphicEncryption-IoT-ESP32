#include "handlers/Rasta6TfheHandler.h"
#include "ciphers/rasta_6_tfhe.h"
#include "Utils.h"
...
const std::string &Rasta6TfheHandler::getResultTopic() const { return result_topic; }
const std::string &Rasta6TfheHandler::getWorkerName() const { return worker_name; }

std::vector<uint8_t> Rasta6TfheHandler::getSecretKey()
{
...
Rasta6TfheHandler::Rasta6TfheHandler()
{
...
void Rasta6TfheHandler::loadContextAndKeys()
{
    std::cout << "[" << worker_name << "] Initializing RASTA6 implementation..." << std::endl;
    std::vector<uint8_t> secret_key = getSecretKey();
    int security_level = 128;

    try
    {
        tfhe_impl_ptr = std::make_unique<RASTA_6::RASTA_128_TFHE>(secret_key, security_level);
    }
    catch (const std::exception &e)
    {
        tfhe_impl_ptr.reset();
        throw std::runtime_error(worker_name + " RASTA6 construction/TFHE setup failed: " + std::string(e.what()));
    }

    std::cout << "[" << worker_name << "] RASTA6 instance created and TFHE context/keys loaded." << std::endl;
}

std::string Rasta6TfheHandler::processMessage(const std::string &ciphertext_hex_payload)
{
    ...
    try
    {
        ...
        std::cout << "[STEP 4] Encrypting rasta6 Key Homomorphically..." << std::endl;
        ...
        std::cout << "[RESULT] rasta6 final result (hex): " << final_result_hex << std::endl;
        ...
        saveMeasurements(
            "measurement/rasta6_tfhe_measurement.csv", // Use correct filename for each handler
            // --- Timers (ms) ---
            keyEncryptTime,
            heDecryptTime,
            heSumTime,
    ...
    catch (const std::exception &e)
    {
        ...
        totalProcessTime = std::chrono::duration_cast<std::chrono::milliseconds>(total_end_time - total_start_time);
        saveMeasurements(
            "measurement/rasta6_tfhe_measurement.csv",
            // --- Timers (ms) ---
            keyEncryptTime,
            heDecryptTime,
            heSumTime,
