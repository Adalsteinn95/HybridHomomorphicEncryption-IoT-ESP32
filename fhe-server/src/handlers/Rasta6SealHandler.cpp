#include "handlers/Rasta6SealHandler.h"
#include "ciphers/rasta_6_seal.h"
#include "Utils.h"
...
const std::string &Rasta6SealHandler::getResultTopic() const { return result_topic; }
const std::string &Rasta6SealHandler::getWorkerName() const { return worker_name; }

// --- Secret Key Retrieval ---
std::vector<uint8_t> Rasta6SealHandler::getSecretKey()
{
...
// --- Constructor ---
Rasta6SealHandler::Rasta6SealHandler() : worker_name("RASTA6_SEAL_WORKER")
{
...
void Rasta6SealHandler::loadContextAndKeys()
{
...
std::string Rasta6SealHandler::processMessage(const std::string &ciphertext_hex_payload)
{
    ...
    try
    {
        ...
        std::unique_ptr<RASTA_6::RASTA_128_SEAL> seal_impl_ptr;
        try
        {
            seal_impl_ptr = std::make_unique<RASTA_6::RASTA_128_SEAL>(secret_key, context);
        }
        catch (const std::exception &e)
        {
...

        {
            ...
            if (!hamming_weight_encrypted_bits.empty())
            {
                ...
                try
                {
                    ...
                    for (size_t i = 0; i < hamming_weight_result.size(); ++i)
                    {
                        std::cout << std::bitset<8>(hamming_weight_result[i]) << " ";
                    }
        ...
        saveMeasurements(
            "measurement/rasta6_seal_measurement.csv", // Use correct filename for each handler
            // --- Timers (ms) ---
            keyEncryptTime,
            heDecryptTime,
            heSumTime,
    ...
    catch (const std::exception &e)
    {
        ...
        totalProcessTime = std::chrono::duration_cast<std::chrono::milliseconds>(total_end_time - total_start_time);
        saveMeasurements(
            "measurement/rasta6_seal_measurement.csv",
            // --- Timers (ms) ---
            keyEncryptTime,
            heDecryptTime,
            heSumTime,
