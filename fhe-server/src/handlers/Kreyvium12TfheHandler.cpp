#include "handlers/Kreyvium12TfheHandler.h"
...
const std::string &Kreyvium12TfheHandler::getResultTopic() const { return result_topic; }
const std::string &Kreyvium12TfheHandler::getWorkerName() const { return worker_name; }

std::vector<uint8_t> Kreyvium12TfheHandler::getSecretKey()
{
...
Kreyvium12TfheHandler::Kreyvium12TfheHandler()
{
...
void Kreyvium12TfheHandler::loadContextAndKeys()
{
    std::cout << "[" << worker_name << "] Initializing Kreyvium12_TFHE implementation..." << std::endl;
    std::vector<uint8_t> secret_key = getSecretKey();
    int security_level = 128;

    try
    {
        tfhe_impl_ptr = std::make_unique<KREYVIUM_12::KREYVIUM12_TFHE>(secret_key, security_level);
    }
    catch (const std::exception &e)
    {
        tfhe_impl_ptr.reset();
        throw std::runtime_error(worker_name + " Kreyvium12_TFHE construction/TFHE setup failed: " + std::string(e.what()));
    }

    std::cout << "[" << worker_name << "] Kreyvium12_TFHE instance created and TFHE context/keys loaded." << std::endl;
}

std::string Kreyvium12TfheHandler::processMessage(const std::string &ciphertext_hex_payload)
{
    ...
    try
    {
        ...
        saveMeasurements(
            "measurement/Kreyvium12_tfhe_measurement.csv", // Use correct filename for each handler
            // --- Timers (ms) ---
            keyEncryptTime,
            heDecryptTime,
            heSumTime,
    ...
    catch (const std::exception &e)
    {
        ...
        totalProcessTime = std::chrono::duration_cast<std::chrono::milliseconds>(total_end_time - total_start_time);
        saveMeasurements(
            "measurement/Kreyvium12_tfhe_measurement.csv",
            // --- Timers (ms) ---
            keyEncryptTime,
            heDecryptTime,
            heSumTime,
