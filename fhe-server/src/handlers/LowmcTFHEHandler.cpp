#include "handlers/LowmcTFHEHandler.h"
#include <iostream>
#include <fstream>
#include <sstream>
#include <vector>
...
const std::string &LowmcTFHEHandler::getResultTopic() const { return result_topic; }
const std::string &LowmcTFHEHandler::getWorkerName() const { return worker_name; }

std::vector<uint8_t> LowmcTFHEHandler::getSecretKey()
{
    std::cout << "[" << worker_name << "] Getting Secret Key..." << std::endl;
    std::cout << "[" << worker_name << " NOTE] Using a hardcoded 16-byte key (128 bits)." << std::endl;
...
LowmcTFHEHandler::LowmcTFHEHandler() : worker_name("LOWMC_TFHE_WORKER")
{
...
void LowmcTFHEHandler::loadContextAndKeys()
{
    std::cout << "[" << worker_name << " Init] Initializing LOWMC_256_128_63_14_TFHE implementation..." << std::endl;
    std::vector<uint8_t> secret_key = getSecretKey();
    int security_level = 128;

    try
    {
        tfhe_impl_ptr = std::make_unique<LOWMC::LOWMC_256_128_63_14_TFHE>(secret_key, security_level);
    }
    catch (const std::exception &e)
    {
        tfhe_impl_ptr.reset();
        throw std::runtime_error(worker_name + " LOWMC_256_128_63_14_TFHE construction/TFHE setup failed: " + std::string(e.what()));
    }

    std::cout << "[" << worker_name << " Init] LOWMC_256_128_63_14_TFHE instance created and TFHE context/keys loaded." << std::endl;
}

std::string LowmcTFHEHandler::processMessage(const std::string &ciphertext_hex_payload)
{
    ...
    try
    {
        ...
        saveMeasurements(
            "measurement/lowmc_tfhe_measurement.csv", // Use correct filename for each handler
            // --- Timers (ms) ---
            keyEncryptTime,
            heDecryptTime,
            heSumTime,
    ...
    catch (const std::exception &e)
    {
        ...
        totalProcessTime = std::chrono::duration_cast<std::chrono::milliseconds>(total_end_time - total_start_time);
        saveMeasurements(
            "measurement/lowmc_tfhe_measurement.csv",
            // --- Timers (ms) ---
            keyEncryptTime,
            heDecryptTime,
            heSumTime,
