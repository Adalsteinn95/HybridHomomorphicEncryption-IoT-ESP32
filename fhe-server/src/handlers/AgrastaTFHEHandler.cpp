#include "handlers/AgrastaTFHEHandler.h"
...
const std::string &AgrastaTFHEHandler::getResultTopic() const { return result_topic; }
const std::string &AgrastaTFHEHandler::getWorkerName() const { return worker_name; }

std::vector<uint8_t> AgrastaTFHEHandler::getSecretKey()
{
...
AgrastaTFHEHandler::AgrastaTFHEHandler()
{
...
void AgrastaTFHEHandler::loadContextAndKeys()
{
    std::cout << "[" << worker_name << "] Initializing AGRASTA_128_TFHE implementation..." << std::endl;
    std::vector<uint8_t> secret_key = getSecretKey();
    int security_level = 128;

    try
    {
        tfhe_impl_ptr = std::make_unique<AGRASTA::AGRASTA_128_TFHE>(secret_key, security_level);
    }
    catch (const std::exception &e)
    {
        tfhe_impl_ptr.reset();
        throw std::runtime_error(worker_name + " AGRASTA_128_TFHE construction/TFHE setup failed: " + std::string(e.what()));
    }

    std::cout << "[" << worker_name << "] AGRASTA_128_TFHE instance created and TFHE context/keys loaded." << std::endl;
}

std::string AgrastaTFHEHandler::processMessage(const std::string &ciphertext_hex_payload)
{
    ...
    try
    {
        ...
        saveMeasurements(
            "measurement/agrasta_tfhe_measurement.csv", // Use correct filename for each handler
            // --- Timers (ms) ---
            keyEncryptTime,
            heDecryptTime,
            heSumTime,
    ...
    catch (const std::exception &e)
    {
        ...
        totalProcessTime = std::chrono::duration_cast<std::chrono::milliseconds>(total_end_time - total_start_time);
        saveMeasurements(
            "measurement/agrasta_tfhe_measurement.csv",
            // --- Timers (ms) ---
            keyEncryptTime,
            heDecryptTime,
            heSumTime,
