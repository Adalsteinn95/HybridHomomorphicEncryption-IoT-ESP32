...
std::vector<uint8_t> LowmcHelibHandler::getSecretKey()
{
...
LowmcHelibHandler::LowmcHelibHandler() : worker_name("LOWMC_WORKER")
{
...
void LowmcHelibHandler::createContextAndKeys()
{
    long p_param_test = 2;
    long r_param_test = 1;
    long L_param_test = 720;
    long c_param_test = 2;
    long d_param_findm_test = 1;
    long k_param_findm_test = 128;
...
std::string LowmcHelibHandler::processMessage(const std::string &ciphertext_hex_payload)
{
    ...
    try
    {
        ...
        std::unique_ptr<LOWMC::LOWMC_256_128_63_14_HElib> helib_impl_ptr;
        unsigned long L = 520;
        unsigned long c = 2;
        std::shared_ptr<helib::Context> shared_context = std::shared_ptr<helib::Context>(std::move(context_));
        try
        {
            helib_impl_ptr = std::make_unique<LOWMC::LOWMC_256_128_63_14_HElib>(secret_key, shared_context, L, c);
        }
        catch (const std::exception &e)
        {
        ...
        saveMeasurements(
            "measurement/lowmc_helib_measurement.csv", // Use correct filename for each handler
            // --- Timers (ms) ---
            keyEncryptTime,
            heDecryptTime,
            heSumTime,
    ...
    catch (const std::exception &e)
    {
        ...
        totalProcessTime = std::chrono::duration_cast<std::chrono::milliseconds>(total_end_time - total_start_time);
        saveMeasurements(
            "measurement/lowmc_helib_measurement.csv",
            // --- Timers (ms) ---
            keyEncryptTime,
            heDecryptTime,
            heSumTime,
