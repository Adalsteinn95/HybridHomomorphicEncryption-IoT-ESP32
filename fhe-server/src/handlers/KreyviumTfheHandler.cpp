#include "handlers/KreyviumTfheHandler.h"
#include "ciphers/kreyvium_tfhe.h"
#include "Utils.h"
...
const std::string &KreyviumTfheHandler::getResultTopic() const { return result_topic; }
const std::string &KreyviumTfheHandler::getWorkerName() const { return worker_name; }

std::vector<uint8_t> KreyviumTfheHandler::getSecretKey()
{
...
KreyviumTfheHandler::KreyviumTfheHandler()
{
...
void KreyviumTfheHandler::loadContextAndKeys()
{
    std::cout << "[" << worker_name << "] Initializing Kreyvium_TFHE implementation..." << std::endl;
    std::vector<uint8_t> secret_key = getSecretKey();
    int security_level = 128;

    try
    {
        tfhe_impl_ptr = std::make_unique<KREYVIUM::KREYVIUM_TFHE>(secret_key, security_level);
    }
    catch (const std::exception &e)
    {
        tfhe_impl_ptr.reset();
        throw std::runtime_error(worker_name + " Kreyvium_TFHE construction/TFHE setup failed: " + std::string(e.what()));
    }

    std::cout << "[" << worker_name << "] Kreyvium_TFHE instance created and TFHE context/keys loaded." << std::endl;
}

std::string KreyviumTfheHandler::processMessage(const std::string &ciphertext_hex_payload)
{
    ...
    try
    {
        ...
        std::cout << "[STEP 4] Encrypting Kreyvium Key Homomorphically..." << std::endl;
        ...
        std::cout << "[RESULT] Kreyvium final result (hex): " << final_result_hex << std::endl;
        ...
        saveMeasurements(
            "measurement/Kreyvium_tfhe_measurement.csv", // Use correct filename for each handler
            // --- Timers (ms) ---
            keyEncryptTime,
            heDecryptTime,
            heSumTime,
    ...
    catch (const std::exception &e)
    {
        ...
        totalProcessTime = std::chrono::duration_cast<std::chrono::milliseconds>(total_end_time - total_start_time);
        saveMeasurements(
            "measurement/Kreyvium_tfhe_measurement.csv",
            // --- Timers (ms) ---
            keyEncryptTime,
            heDecryptTime,
            heSumTime,
