#include "handlers/Filip1280SealHandler.h"
...
const std::string &Filip1280SealHandler::getResultTopic() const { return result_topic; }
const std::string &Filip1280SealHandler::getWorkerName() const { return worker_name; }

std::vector<uint8_t> Filip1280SealHandler::getSecretKey()
{
    std::cout << "[" << worker_name << "] Getting Secret Key..." << std::endl;
    std::vector<uint8_t> key(512, 0xAA);
    return key;
}

Filip1280SealHandler::Filip1280SealHandler() : worker_name("FILIP_1280_SEAL_WORKER") // Use a consistent worker name
{
...
void Filip1280SealHandler::loadContextAndKeys()
{
    ...
    {
        throw std::runtime_error("Failed to create SEALContext for FiLIP-1280: " + std::string(e.what()));
    }
    if (!context->parameters_set())
        throw std::runtime_error("FiLIP-1280 SEAL context parameters check failed.");
...
std::string Filip1280SealHandler::processMessage(const std::string &ciphertext_hex_payload)
{
    ...
    try
    {
        ...
        std::unique_ptr<FILIP_1280::FiLIP_1280_SEAL> seal_impl_ptr;
        try
        {
            seal_impl_ptr = std::make_unique<FILIP_1280::FiLIP_1280_SEAL>(secret_key, context);
        }
        catch (const std::exception &e)
        {
            throw std::runtime_error(worker_name + " AGRASTA_128_SEAL construction failed: " + std::string(e.what()));
        }
        ...
        std::cout << "[STEP 4] Encrypting AGRASTA Key Homomorphically..." << std::endl;
        ...
        std::cout << "[RESULT] Agrasta final result (hex): " << final_result_hex << std::endl;
        ...
        saveMeasurements(
            "measurement/filip1280_seal_measurement.csv", // Use correct filename for each handler
            // --- Timers (ms) ---
            keyEncryptTime,
            heDecryptTime,
            heSumTime,
    ...
    catch (const std::exception &e)
    {
        ...
        totalProcessTime = std::chrono::duration_cast<std::chrono::milliseconds>(total_end_time - total_start_time);
        saveMeasurements(
            "measurement/filip1280_seal_measurement.csv",
            // --- Timers (ms) ---
            keyEncryptTime,
            heDecryptTime,
            heSumTime,
