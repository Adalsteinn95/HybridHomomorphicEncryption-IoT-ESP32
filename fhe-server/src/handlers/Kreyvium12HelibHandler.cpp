...
#include "kreyvium_12_helib.h" // For KREYVIUM_12::KREYVIUM12_HElib
...
std::vector<uint8_t> Kreyvium12HelibHandler::getSecretKey()
{
    std::cout << "[" << worker_name << "] Using a hardcoded 16-byte secret key for KREYVIUM." << std::endl;
    std::vector<uint8_t> key = {// Using the 16-byte key from Kreyvium12SealHandler
                                0x0c, 0x7c, 0xc2, 0x27, 0xec, 0xed, 0x1e, 0x4f,
                                0x33, 0xdb, 0xe4, 0xab, 0xdd, 0xfb, 0xb3, 0xbd};
    return key;
}

Kreyvium12HelibHandler::Kreyvium12HelibHandler() : worker_name("KREYVIUM12_HELIB_WORKER")
{
...
void Kreyvium12HelibHandler::createContextAndKeys()
{
    long p_param_test = 2;
    long r_param_test = 1;
    long L_param_test = 395;
    long c_param_test = 2;
...
std::string Kreyvium12HelibHandler::processMessage(const std::string &ciphertext_hex_payload)
{
    ...
    try
    {
        ...
        std::unique_ptr<KREYVIUM_12::KREYVIUM12_HElib> helib_impl_ptr;
        unsigned long L = 520;
        unsigned long c = 2;
        std::shared_ptr<helib::Context> shared_context = std::shared_ptr<helib::Context>(std::move(context_));
        try
        {
            helib_impl_ptr = std::make_unique<KREYVIUM_12::KREYVIUM12_HElib>(secret_key, shared_context, L, c);
        }
        catch (const std::exception &e)
        {
            throw std::runtime_error(worker_name + " kreyvium12_128_SEAL construction failed: " + std::string(e.what()));
        }
        ...
        saveMeasurements(
            "measurement/kreyvium12_helib_measurement.csv", // Use correct filename for each handler
            // --- Timers (ms) ---
            keyEncryptTime,
            heDecryptTime,
            heSumTime,
    ...
    catch (const std::exception &e)
    {
        ...
        totalProcessTime = std::chrono::duration_cast<std::chrono::milliseconds>(total_end_time - total_start_time);
        saveMeasurements(
            "measurement/kreyvium12_helib_measurement.csv",
            // --- Timers (ms) ---
            keyEncryptTime,
            heDecryptTime,
            heSumTime,
