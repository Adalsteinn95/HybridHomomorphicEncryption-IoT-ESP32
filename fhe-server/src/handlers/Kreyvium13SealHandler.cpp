#include "handlers/Kreyvium13SealHandler.h"
#include "ciphers/kreyvium_13_seal.h"
#include "Utils.h"
...
std::vector<uint8_t> Kreyvium13SealHandler::getSecretKey()
{
...
const std::string &Kreyvium13SealHandler::getResultTopic() const { return result_topic; }
const std::string &Kreyvium13SealHandler::getWorkerName() const { return worker_name; }

Kreyvium13SealHandler::Kreyvium13SealHandler() : worker_name("KREYVIUM13_SEAL_WORKER")
{
...
void Kreyvium13SealHandler::loadContextAndKeys()
{
...
std::string Kreyvium13SealHandler::processMessage(const std::string &ciphertext_hex_payload)
{
    std::string final_result_hex;
    std::string last_correctness_status = "UNKNOWN";
    for (int run = 0; run < 3; ++run)
    {
        std::cout << "\n[RUN " << (run + 1) << "/3] ==============================" << std::endl;
        std::string result_hex;
        std::string correctness_status = "UNKNOWN";
        try
        {
            result_hex = ""; // Default in case of error
            // Copy-paste the original function body here, but replace all "return final_result_hex;" with "result_hex = final_result_hex;" and break;
            // Also, replace all "return ..." in the catch block with "result_hex = ...;" and break;
            // To avoid code duplication, we can call the original logic as a lambda:

            auto process_once = [&](std::string &correctness_status) -> std::string
            {
                ...
                try
                {
                    ...
                    std::unique_ptr<KREYVIUM_13::KREYVIUM13_SEAL> seal_impl_ptr;
                    try
                    {
                        seal_impl_ptr = std::make_unique<KREYVIUM_13::KREYVIUM13_SEAL>(secret_key, context);
                    }
                    catch (const std::exception &e)
                    {
                        throw std::runtime_error(worker_name + " Kreyvium13 construction failed: " + std::string(e.what()));
                    }
                    ...
                    saveMeasurements(
                        "measurement/kreyvium13_seal_measurement.csv", // Use correct filename for each handler
                        // --- Timers (ms) ---
                        keyEncryptTime,
                        heDecryptTime,
                        heSumTime,
                ...
                catch (const std::exception &e)
                {
                    ...
                    totalProcessTime = std::chrono::duration_cast<std::chrono::milliseconds>(total_end_time - total_start_time);
                    saveMeasurements(
                        "measurement/kreyvium13_seal_measurement.csv",
                        // --- Timers (ms) ---
                        keyEncryptTime,
                        heDecryptTime,
                        heSumTime,
            ...
            result_hex = process_once(correctness_status);
        }
        catch (const std::exception &e)
        {
            std::cerr << "[RUN " << (run + 1) << "] Exception: " << e.what() << std::endl;
            result_hex = "Error: Unexpected exception occurred.";
            correctness_status = "EXCEPTION";
        }
        final_result_hex = result_hex;
        last_correctness_status = correctness_status;
    }
    return final_result_hex;
}