...
#include "kreyvium_helib.h" // For KREYVIUM_12::KREYVIUM12_HElib
...
std::vector<uint8_t> KreyviumHelibHandler::getSecretKey()
{
...
KreyviumHelibHandler::KreyviumHelibHandler() : worker_name("KREYVIUM13_HELIB_WORKER")
{
...
void KreyviumHelibHandler::createContextAndKeys()
{
    long p_param_test = 2;
    long r_param_test = 1;
...
std::string KreyviumHelibHandler::processMessage(const std::string &ciphertext_hex_payload)
{
    ...
    try
    {
        ...
        std::unique_ptr<KREYVIUM::KREYVIUM_HElib> helib_impl_ptr;
        unsigned long L = 520;
        unsigned long c = 2;
        std::shared_ptr<helib::Context> shared_context = std::shared_ptr<helib::Context>(std::move(context_));
        try
        {
            helib_impl_ptr = std::make_unique<KREYVIUM::KREYVIUM_HElib>(secret_key, shared_context, L, c);
        }
        catch (const std::exception &e)
        {
            throw std::runtime_error(worker_name + " kreyvium_128_SEAL construction failed: " + std::string(e.what()));
        }
        ...
        saveMeasurements(
            "measurement/kreyvium_helib_measurement.csv", // Use correct filename for each handler
            // --- Timers (ms) ---
            keyEncryptTime,
            heDecryptTime,
            heSumTime,
    ...
    catch (const std::exception &e)
    {
        ...
        totalProcessTime = std::chrono::duration_cast<std::chrono::milliseconds>(total_end_time - total_start_time);
        saveMeasurements(
            "measurement/kreyvium_helib_measurement.csv",
            // --- Timers (ms) ---
            keyEncryptTime,
            heDecryptTime,
            heSumTime,
