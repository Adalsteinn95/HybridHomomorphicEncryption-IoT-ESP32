...
std::vector<uint8_t> Filip1280HelibHandler::getSecretKey()
{
...
Filip1280HelibHandler::Filip1280HelibHandler() : worker_name("filip1280_HELIB_WORKER")
{
...
void Filip1280HelibHandler::createContextAndKeys()
{
    long p_param_test = 2;
    long r_param_test = 1;
...
std::string Filip1280HelibHandler::processMessage(const std::string &ciphertext_hex_payload)
{
    ...
    try
    {
        ...
        std::unique_ptr<FILIP_1280::FiLIP_1280_HElib> helib_impl_ptr;
        unsigned long L = 520;
        unsigned long c = 2;
        std::shared_ptr<helib::Context> shared_context = std::shared_ptr<helib::Context>(std::move(context_));
        try
        {
            helib_impl_ptr = std::make_unique<FILIP_1280::FiLIP_1280_HElib>(secret_key, shared_context, L, c);
        }
        catch (const std::exception &e)
        {
            throw std::runtime_error(worker_name + " filip1280 failed: " + std::string(e.what()));
        }
        ...
        saveMeasurements(
            "measurement/filip1280_helib_measurement.csv", // Use correct filename for each handler
            // --- Timers (ms) ---
            keyEncryptTime,
            heDecryptTime,
            heSumTime,
    ...
    catch (const std::exception &e)
    {
        ...
        totalProcessTime = std::chrono::duration_cast<std::chrono::milliseconds>(total_end_time - total_start_time);
        saveMeasurements(
            "measurement/filip1280_helib_measurement.csv",
            // --- Timers (ms) ---
            keyEncryptTime,
            heDecryptTime,
            heSumTime,
