#include "handlers/KreyviumSealHandler.h"
#include "ciphers/kreyvium_seal.h"
#include "Utils.h"
...
const std::string &KreyviumSealHandler::getResultTopic() const { return result_topic; } // Assuming result_topic is a member
const std::string &KreyviumSealHandler::getWorkerName() const { return worker_name; }   // worker_name is already a member

// --- Secret Key Retrieval ---
std::vector<uint8_t> KreyviumSealHandler::getSecretKey()
{
...
// --- Constructor  ---
KreyviumSealHandler::KreyviumSealHandler() : worker_name("KREYVIUM_SEAL_WORKER") // Use a consistent worker name
{
...
void KreyviumSealHandler::loadContextAndKeys()
{
...
std::string KreyviumSealHandler::processMessage(const std::string &ciphertext_hex_payload)
{
    ...
    try
    {
        ...
        std::unique_ptr<KREYVIUM::KREYVIUM_SEAL> seal_impl_ptr;
        try
        {
            seal_impl_ptr = std::make_unique<KREYVIUM::KREYVIUM_SEAL>(secret_key, context);
        }
        catch (const std::exception &e)
        {
            throw std::runtime_error(worker_name + " Kreyvium construction failed: " + std::string(e.what()));
        }
        ...
        saveMeasurements(
            "measurement/kreyvium_seal_measurement.csv", // Use correct filename for each handler
            // --- Timers (ms) ---
            keyEncryptTime,
            heDecryptTime,
            heSumTime,
    ...
    catch (const std::exception &e)
    {
        ...
        totalProcessTime = std::chrono::duration_cast<std::chrono::milliseconds>(total_end_time - total_start_time);
        saveMeasurements(
            "measurement/kreyvium_seal_measurement.csv",
            // --- Timers (ms) ---
            keyEncryptTime,
            heDecryptTime,
            heSumTime,
