#include "handlers/Rasta5TfheHandler.h"
...
const std::string &Rasta5TfheHandler::getResultTopic() const { return result_topic; }
const std::string &Rasta5TfheHandler::getWorkerName() const { return worker_name; }

std::vector<uint8_t> Rasta5TfheHandler::getSecretKey()
{
...
Rasta5TfheHandler::Rasta5TfheHandler()
{
...
void Rasta5TfheHandler::loadContextAndKeys()
{
    std::cout << "[" << worker_name << "] Initializing RASTA5 implementation..." << std::endl;
    std::vector<uint8_t> secret_key = getSecretKey();
    int security_level = 128;

    try
    {
        tfhe_impl_ptr = std::make_unique<RASTA_5::RASTA_128_TFHE>(secret_key, security_level);
    }
    catch (const std::exception &e)
    {
        tfhe_impl_ptr.reset();
        throw std::runtime_error(worker_name + " RASTA5 construction/TFHE setup failed: " + std::string(e.what()));
    }

    std::cout << "[" << worker_name << "] RASTA5 instance created and TFHE context/keys loaded." << std::endl;
}

std::string Rasta5TfheHandler::processMessage(const std::string &ciphertext_hex_payload)
{
    std::string final_result_hex = "";
    std::string last_correctness_status = "UNKNOWN";
    for (int run = 0; run < 4; ++run)
    {
        std::cout << "\n[RUN " << (run + 1) << "/4] ==============================" << std::endl;
        ...
        try
        {
            ...
            saveMeasurements(
                "measurement/rasta5_tfhe_measurement.csv", // Use correct filename for each handler
                // --- Timers (ms) ---
                keyEncryptTime,
                heDecryptTime,
                heSumTime,
        ...
        catch (const std::exception &e)
        {
            ...
            totalProcessTime = std::chrono::duration_cast<std::chrono::milliseconds>(total_end_time - total_start_time);
            saveMeasurements(
                "measurement/rasta5_tfhe_measurement.csv",
                // --- Timers (ms) ---
                keyEncryptTime,
                heDecryptTime,
                heSumTime,
            ...
            last_correctness_status = correctness_status;
            final_result_hex = "Error: Unexpected exception occurred.";
        }
    }
