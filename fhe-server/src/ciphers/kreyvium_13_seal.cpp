...
namespace KREYVIUM_13
{
  ...
  E_BIT_13_SEAL &E_BIT_13_SEAL::operator=(const E_BIT_13_SEAL &o) noexcept
  {
    this->ct = o.ct;
    this->pt = o.pt;
  ...
  E_BIT_13_SEAL &E_BIT_13_SEAL::operator=(const Ciphertext &o) noexcept
  {
    this->ct = o;
    this->pt = false;
  ...
  E_BIT_13_SEAL &E_BIT_13_SEAL::operator=(const bool o) noexcept
  {
    this->pt = o;
  ...
  void E_BIT_13_SEAL::XOR(const E_BIT_13_SEAL &b1, const E_BIT_13_SEAL &b2, E_BIT_13_SEAL &r,
                          seal::Evaluator &eval)
  {
  ...
  void E_BIT_13_SEAL::XOR_inplace(E_BIT_13_SEAL &r, const E_BIT_13_SEAL &b, seal::Evaluator &eval)
  {
  ...
  void E_BIT_13_SEAL::XOR_inplace(E_BIT_13_SEAL &r, const Ciphertext &c, seal::Evaluator &eval)
  {
    if (r.is_encrypted)
    {
  ...
  void E_BIT_13_SEAL::XOR_plain_inplace(E_BIT_13_SEAL &r, const bool &b, seal::Evaluator &eval)
  {
    if (r.is_encrypted)
    {
      if (!b)
  ...
  void E_BIT_13_SEAL::AND(const E_BIT_13_SEAL &b1, const E_BIT_13_SEAL &b2, E_BIT_13_SEAL &r,
  ...
  void E_BIT_13_SEAL::AND_inplace(E_BIT_13_SEAL &r, const E_BIT_13_SEAL &b, seal::Evaluator &eval,
                                  seal::RelinKeys &rk)
  {
    if (r.is_encrypted && b.is_encrypted)
    {
  ...
  void KREYVIUM13_SEAL::encrypt_key()
  {
  ...
  std::vector<Ciphertext> KREYVIUM13_SEAL::HE_decrypt(
      std::vector<uint8_t> &ciphertexts, size_t bits)
  {
    ...
    std::vector<E_BIT_13_SEAL> state(STATE_SIZE);
    ivblock inner_iv;
    ...
    for (size_t b = 0; b < num_block; b++)
    {
      ...
      {
        Ciphertext t4 = inner_key[KREYVIUM13_PARAMS.key_size_bits - 1];
        bool t5 = inner_iv[IV_SIZE - 1];
        E_BIT_13_SEAL t1;
        E_BIT_13_SEAL::XOR(state[65], state[92], t1, evaluator);
        E_BIT_13_SEAL t2;
        E_BIT_13_SEAL::XOR(state[161], state[176], t2, evaluator);
        E_BIT_13_SEAL t3;
        E_BIT_13_SEAL::XOR(state[242], state[287], t3, evaluator);
        E_BIT_13_SEAL::XOR_inplace(t3, t4, evaluator);
        if (i >= warmup)
        {
        ...
        E_BIT_13_SEAL tmp;
        E_BIT_13_SEAL::XOR_inplace(t1, state[170], evaluator);
        E_BIT_13_SEAL::XOR_plain_inplace(t1, t5, evaluator);
        E_BIT_13_SEAL::AND(state[90], state[91], tmp, evaluator, he_rk);
        E_BIT_13_SEAL::XOR_inplace(t1, tmp, evaluator);

        E_BIT_13_SEAL::XOR_inplace(t2, state[263], evaluator);
        E_BIT_13_SEAL::AND(state[174], state[175], tmp, evaluator, he_rk);
        E_BIT_13_SEAL::XOR_inplace(t2, tmp, evaluator);

        E_BIT_13_SEAL::XOR_inplace(t3, state[68], evaluator);
        E_BIT_13_SEAL::AND(state[285], state[286], tmp, evaluator, he_rk);
        E_BIT_13_SEAL::XOR_inplace(t3, tmp, evaluator);
        inner_key.pop_back();
        state.pop_back();
        inner_iv <<= 1;
  ...
  std::vector<uint8_t> KREYVIUM13_SEAL::decrypt_result(
