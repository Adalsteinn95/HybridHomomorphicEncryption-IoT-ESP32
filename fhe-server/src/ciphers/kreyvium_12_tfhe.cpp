...
namespace KREYVIUM_12
{
  ...
  E_BIT_12_TFHE &E_BIT_12_TFHE::operator=(const E_BIT_12_TFHE &o) noexcept
  {
    this->ct = o.ct;
    this->pt = o.pt;
  ...
  E_BIT_12_TFHE &E_BIT_12_TFHE::operator=(const TFHECiphertext &o) noexcept
  {
    this->ct = o;
    this->pt = false;
  ...
  E_BIT_12_TFHE &E_BIT_12_TFHE::operator=(const bool o) noexcept
  {
    this->pt = o;
  ...
  E_BIT_12_TFHE &E_BIT_12_TFHE::operator=(LweSample &o) noexcept
  {
    this->ct = o;
    this->pt = false;
    this->is_encrypted = true;
  ...
  void E_BIT_12_TFHE::XOR(const E_BIT_12_TFHE &b1, const E_BIT_12_TFHE &b2, E_BIT_12_TFHE &r,
  ...
  void E_BIT_12_TFHE::XOR_inplace(E_BIT_12_TFHE &r, const E_BIT_12_TFHE &b,
                                  const TFheGateBootstrappingCloudKeySet *pk)
  {
  ...
  void E_BIT_12_TFHE::XOR_inplace(E_BIT_12_TFHE &r, const TFHECiphertext &c,
                                  const TFheGateBootstrappingCloudKeySet *pk)
  {
    if (r.is_encrypted)
    {
  ...
  void E_BIT_12_TFHE::XOR_plain_inplace(E_BIT_12_TFHE &r, const bool &b,
                                        const TFheGateBootstrappingCloudKeySet *pk)
  {
    if (r.is_encrypted)
    {
  ...
  void E_BIT_12_TFHE::AND(const E_BIT_12_TFHE &b1, const E_BIT_12_TFHE &b2, E_BIT_12_TFHE &r,
                          const TFheGateBootstrappingCloudKeySet *pk)
  {
    if (b1.is_encrypted && b2.is_encrypted)
    {
  ...
  void E_BIT_12_TFHE::AND_inplace(E_BIT_12_TFHE &r, const E_BIT_12_TFHE &b,
                                  const TFheGateBootstrappingCloudKeySet *pk)
  {
    if (r.is_encrypted && b.is_encrypted)
    {
                                                ...
                                                size_t bits)
  {
    ...
    std::vector<E_BIT_12_TFHE> state(STATE_SIZE, context);
    ...
    for (size_t b = 0; b < num_block; b++)
    {
      ...
      for (size_t i = 0; i < warmup + KREYVIUM12_PARAMS.plain_size_bits; i++)
      {
        ...
        E_BIT_12_TFHE t1(context);
        E_BIT_12_TFHE::XOR(state[65], state[92], t1, he_pk);
        E_BIT_12_TFHE t2(context);
        E_BIT_12_TFHE::XOR(state[161], state[176], t2, he_pk);
        E_BIT_12_TFHE t3(context);
        E_BIT_12_TFHE::XOR(state[242], state[287], t3, he_pk);
        E_BIT_12_TFHE::XOR_inplace(t3, t4, he_pk);
        if (i >= warmup)
        {
        ...
        E_BIT_12_TFHE tmp(context);
        E_BIT_12_TFHE::XOR_inplace(t1, state[170], he_pk);
        E_BIT_12_TFHE::XOR_plain_inplace(t1, t5, he_pk);
        E_BIT_12_TFHE::AND(state[90], state[91], tmp, he_pk);
        E_BIT_12_TFHE::XOR_inplace(t1, tmp, he_pk);

        E_BIT_12_TFHE::XOR_inplace(t2, state[263], he_pk);
        E_BIT_12_TFHE::AND(state[174], state[175], tmp, he_pk);
        E_BIT_12_TFHE::XOR_inplace(t2, tmp, he_pk);

        E_BIT_12_TFHE::XOR_inplace(t3, state[68], he_pk);
        E_BIT_12_TFHE::AND(state[285], state[286], tmp, he_pk);
        E_BIT_12_TFHE::XOR_inplace(t3, tmp, he_pk);
        state.pop_back();
        inner_key.pop_back();
        inner_iv <<= 1;
