...
public:
     ...
     */
    template <typename T>
    size_t get_helib_object_size_bytes(const T &obj) const;

    size_t get_ciphertexts_size_bytes(const std::vector<helib::Ctxt> &ciphs) const;
    ...
    e_int computeHammingWeight(const std::vector<helib::Ctxt> &bits);
};
