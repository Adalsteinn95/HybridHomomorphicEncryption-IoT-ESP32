...
public:
  ...
  TFheGateBootstrappingParameterSet *get_params() const
  {
    return params;
  }
...
public:
  ...
  e_int computeHammingWeight(const TFHECiphertextVec &bits);
  ...
  /**
   * @brief Calculates the total estimated memory size of a TFHECiphertextVec.
   * @param ciphs The TFHE ciphertext vector wrapper.
   * @return The total estimated size in bytes.
   */
  size_t get_ciphertexts_size_bytes(const TFHECiphertextVec &ciphs) const;
};
